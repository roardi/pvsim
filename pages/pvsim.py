...
# create sources
with open("Output_pv.txt", "w") as text_file:
...
##        new_data = " ".join(data_stop)
        with open("Output_pv.txt", "a") as text_file:
            text_file.writelines(data1)
            text_file.writelines("\n")
            
train_sources = {'Output_pv.txt':'TRAIN', 'Test.txt':'TEST'}

##log.info('TaggedDocument')
sentences = TaggedLineSentence(train_sources)

##log.info('D2V')
model = Doc2Vec(min_count=1, window=10, size=200, workers=3, alpha=0.025, iter=5)
model.build_vocab(sentences)

model.train(sentences)

##log.info('Model Save')
model.save('./pvsim.d2v')
model = Doc2Vec.load('./pvsim.d2v')
...
##    new_row = " ".join(row_stop)
    ...
    with open("Data_pv.txt", "w") as text_file:
