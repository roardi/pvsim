...
from gensim.models.doc2vec_tf import TaggedDocument
from gensim.models import Doc2Vec_tf
...
with open("Test_tf.txt", "w") as text_file:
...
# create sources
with open("Output_tf.txt", "w") as text_file:
...
train_sources = {'Output_tf.txt':'TRAIN', 'Test_tf.txt':'TEST'}

##log.info('TaggedDocument')
sentences = TaggedLineSentence(train_sources)

##log.info('D2V')
model = Doc2Vec_tf(min_count=1, window=10, size=100, workers=3, alpha=0.025, iter=5)
model.build_vocab(sentences)

model.train(sentences)

##log.info('Model Save')
model.save('./pvsim_tf.d2v')
model = Doc2Vec_tf.load('./pvsim_tf.d2v')
...
##    new_row = " ".join(row_stop)
    ...
    with open("Data_tf.txt", "w") as text_file:
