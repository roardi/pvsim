...
from gensim.models.doc2vec_atf import TaggedDocument
from gensim.models import Doc2Vec_atf
...
with open("Test_atf.txt", "w") as text_file:
...
# create sources
with open("Output_atf.txt", "w") as text_file:
...
##        new_data = " ".join(data_stop)
        with open("Output_atf.txt", "a") as text_file:
            text_file.writelines(data1)
            text_file.writelines("\n")
            
train_sources = {'Output_atf.txt':'TRAIN', 'Test_atf.txt':'TEST'}

##log.info('TaggedDocument')
sentences = TaggedLineSentence(train_sources)

##log.info('D2V')
model = Doc2Vec_atf(min_count=1, window=10, size=100, workers=3, alpha=0.025, iter=10)
model.build_vocab(sentences)

model.train(sentences)

##log.info('Model Save')
model.save('./pvsim_atf.d2v')
model = Doc2Vec_atf.load('./pvsim_atf.d2v')
...
##    new_row = " ".join(row_stop)
    ...
    with open("Data_atf.txt", "w") as text_file:
