...
try:
    from gensim.models.doc2vec_atf import train_document_dbow, train_document_dm, train_document_dm_concat
    ...
    def train_document_dm(model, doc_words, doctag_indexes, alpha, work=None, neu1=None, weight=None,
        ...
        raw = open("C:\Python27\word2vec\keyword.txt", "rU")
        raw_read = raw.read()
        keyword = nltk.word_tokenize(raw_read)
        
        for pos, word in enumerate(word_vocabs):
##            print word.word,model.weight[word.word]
...
##            if word.word in keyword:
            l1 *= model.weight[word.word] * model.doc_weight
...
class Doc2Vec_atf(Word2Vec):
    ...
    def __init__(self, documents=None, size=300, alpha=0.025, window=8, min_count=5,
        ...
        """
        super(Doc2Vec_atf, self).__init__(
    ...
    def clear_sims(self):
        super(Doc2Vec_atf, self).clear_sims()
    ...
    def reset_weights(self):
        ...
        super(Doc2Vec_atf, self).reset_weights()
    ...
    def reset_from(self, other_model):
        ...
        super(Doc2Vec_atf, self).reset_from(other_model)
    ...
    def scan_vocab(self, documents, progress_per=10000, trim_rule=None):
        ...
        logger.info("from document collected %i word types and %i unique tags from a corpus of %i examples and %i words",
...
##            for word in doc.words:
##                if word in keyword:
##                    w_word = tf[word] / float(len(doc.words))
##                else:
##                    w_word = 1 / float(len(doc.words))
##                
##                weight[word] += w_word
##            self.weight = weight
...
##                    w_word = 1
            ...
            self.doc_weight = doc_weight / len(doc)
...
##            weight.clear()
##            tf.clear()
##            avdl = float(len(doc.words)) / len_doc
##            for word in doc.words:
##                if word in keyword:
##                    tf[word] += 1
##                else:
...
##                if word in keyword:
##                    a = (1.2 + 1) * tf[word]
##                    b = (1 - 0.95) + (0.95 * len(doc.words) / avdl)
##                    c = 1.2 * b + tf[word]
##                    w_word = a / c
##                else:
##                    w_word = 1
...
##            self.weight = weight
            ...
            if self.sg:
                ...
                tally += train_document_dm(self, doc.words, doctag_indexes, alpha, work, neu1, weight,
                                           doctag_vectors=doctag_vectors, doctag_locks=doctag_locks)
##            print "=======batas doc========="
    ...
    def estimate_memory(self, vocab_size=None, report=None):
        ...
        return super(Doc2Vec_atf, self).estimate_memory(vocab_size, report=report)
